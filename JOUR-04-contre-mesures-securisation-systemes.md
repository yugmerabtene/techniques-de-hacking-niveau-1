...
|---|---|---|---|
| 1h30 | secure-linux-target (port 2224) | `rendu_labs/jour-04/` | M1051 + M1037 + M1036 + M1050 + M1022 |
...
mkdir -p rendu_labs/jour-04 && cd rendu_labs/jour-04
```
...
> **📌 Ce qu'on a retenu :** Durcissement Linux complet en 7 étapes (mise à jour, désactivation de services, durcissement SSH, pare-feu UFW, fail2ban, protections noyau ASLR, audit SUID). Chaque étape correspond à une mitigation ATT&CK et s'aligne sur les recommandations ANSSI.  
> **Attendu :** SSH root refusé, UFW actif avec politique deny incoming, fail2ban configuré (maxretry=3, bantime=3600), ASLR en mode full randomization (valeur 2).  
> **Défense :** M1051 (Updates) + M1018 (Account Mgmt) + M1037 (Network Filtering) + M1036 (Brute Force Protection) + M1050 (Exploit Protection) + M1022 (File Integrity).
...
| Durée | Conteneur | Dossier | Mitigations |
|---|---|---|---|
| 1h | elk-soc (Kibana 5601) | `rendu_labs/jour-04/` | M1047 Log Collection + M1030 SIEM |
...
> **📌 Ce qu'on a retenu :** Centralisation des logs avec ELK Stack (Filebeat → Logstash → Elasticsearch → Kibana). Le SOC peut détecter les attaques (SQLi, brute-force, scan nmap, backdoor) en temps réel grâce aux règles de détection.  
> **Attendu :** Logs Apache visibles dans Kibana Discover, tableau de bord de sécurité fonctionnel, détection SQLi par requête Kibana.  
> **Défense :** M1047 (Log Collection) + M1030 (SIEM).
...
| Lab | Attaque ciblée | Compétence acquise | Mitigation ATT&CK |
|-----|---------------|-------------------|-------------------|
| 4.1 — Durcissement Linux | Brute-force SSH (T1110), Buffer Overflow (T1068), Scan (T1046) | Hardening en 7 étapes : mise à jour, SSH, UFW, fail2ban, ASLR, audit SUID | M1051 + M1018 + M1037 + M1036 + M1050 + M1022 |
| 4.2 — SOC / ELK Stack | SQLi (T1190), Brute-force (T1110), Scan (T1046), Backdoor vsftpd (T1190) | Centralisation des logs avec Filebeat + Logstash + Elasticsearch + Kibana, détection temps réel, dashboard SOC | M1047 + M1030 |

### La défense en profondeur

La défense en profondeur (defense in depth) est le principe fondamental qui relie ces deux labs. Aucune mesure seule ne suffit : le durcissement réduit la surface d'attaque, mais ne détecte pas une intrusion réussie. Le SOC détecte les attaques, mais ne les empêche pas. C'est la combinaison des deux — prévention + détection — qui crée une posture de sécurité résiliente.

Dans un contexte réglementaire français (RGS, NIS2), ce chapitre montre comment aligner des mesures techniques concrètes avec les exigences d'homologation. La matrice de couverture défensive et l'évaluation des risques via le triangle CIA sont les outils qui permettent de prioriser les investissements de sécurité et d'identifier les angles morts avant qu'ils ne soient exploités.
